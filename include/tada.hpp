...
namespace tada
{
    ...
    /** Default constructor */
    template <typename T>
    portable Derivable<T>::Derivable() = default;
...
/** Constructor for `Derivable`s with specifier and different types */
    ...
    portable Derivable<T>::Derivable(const S &v, const S &d, const S &dd)
        : value(static_cast<T>(v)), deriv(static_cast<T>(d)), dderiv(static_cast<T>(dd))
    {
    }
#endif
    ...
    {
        T xdy = x.v() / y.v();
        T _f1 = (x.d() - xdy * y.d()) / y.v();
        return Derivable<T>(
            xdy, _f1
#if TADA_ORDER > 1
            ,
            (x.dd() - xdy * y.dd() - two<T>() * _f1 * y.d()) / y.v()
#endif
        );
    }
